...
import { Plus, FileDown, Store } from 'lucide-react';
...
export default function FraisMagasinPage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const response = await api.getCategoriesFrais({ type_categorie: 'Magasin' });
      if (response.success) {
  ...
  const fetchFrais = useCallback(async () => {
    try {
      setLoading(true);
      const currentFilters: any = { ...filters, type_categorie: 'Magasin' };
  ...
  const fetchStats = useCallback(async () => {
    try {
      const currentFilters: any = { type_categorie: 'Magasin' };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      a.href = url;
      a.download = `frais_magasin_${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gérez les dépenses du magasin (WiFi, électricité, salaires, etc.)
          </p>
        </div>
