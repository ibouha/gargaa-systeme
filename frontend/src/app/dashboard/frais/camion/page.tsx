...
export default function FraisCamionPage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      const response = await api.getCategoriesFrais({ type_categorie: 'Camion' });
      if (response.success) {
  ...
  const fetchFrais = useCallback(async () => {
    try {
      setLoading(true);
      const currentFilters: any = { ...filters, type_categorie: 'Camion' };
  ...
  const fetchStats = useCallback(async () => {
    try {
      const currentFilters: any = { type_categorie: 'Camion' };
  ...
  const handleExportPDF = async () => {
    try {
      ...
      a.href = url;
      a.download = `frais_camion_${new Date().toISOString().split('T')[0]}.pdf`;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gérez les dépenses des camions (gasoil, entretien, salaires chauffeurs, etc.)
          </p>
        </div>
